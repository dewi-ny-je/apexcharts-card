...
| `unit` | boolean or string | `true` | v2.3.0 | Only valid if `in_header: true`. If `false`, the unit of measurement is not shown next to the state in the header. If set to `before_value`, the unit is shown before the state instead of after it (eg. to display `$ 1.50` instead of `1.50 $`) |
